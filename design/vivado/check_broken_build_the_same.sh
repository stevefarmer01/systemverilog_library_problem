...
rm md5sum_broken_blank_id_version.txt
md5sum < project_1_broken/project_1_broken.runs/impl_1/systemverilog_library_problem_top.bin > md5sum_broken_blank_id_version.txt
...
cat md5sum_broken_blank_id_version.txt
printf "\nChecking current and previous .bin files are the same....\n\n"
diff -s saved_md5sum_blank_id_version.txt md5sum_broken_blank_id_version.txt
if [ $? -eq 0 ]
then
